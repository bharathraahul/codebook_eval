...
def run_llm_with_codebook(limit=None):
    ...
    return run_experiment("LLM With Codebook", prompt_fn,
                          f'{REPO}/outputs/aw_llm_cb.csv', limit=limit)
...
def print_table():
    ...
    for name, csv_name in [
        ...
        ('LLM With Codebook', 'aw_llm_cb.csv'),
    ...
    print(f"{'─'*42}")
    print(f"  {'[Paper] GPT-3.5':<28} {'76.3':>8}")
    print(f"  {'[Paper] GPT-4':<28} {'87.0':>8}")
    print(f"  {'[Paper] ZSP Tree':<28} {'88.0':>8}")
...
def main():
    ...
    if args.step not in ('table', 'all'):
        print(f"\n  → Run 'python3 AW_Experiments.py --step table' for comparison.")
