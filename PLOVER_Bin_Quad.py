...
def run_llm_bin_cb(limit=None):
    def prompt_fn(sentence):
        return (f"You are a political event classifier. Classify the relation "
                ...
                f"Sentence: {sentence}\n\n"
                f"Output ONLY one label: COOPERATION or CONFLICT")
    return run_llm_binary_experiment("LLM Bin CB", prompt_fn,
                              f'{REPO}/outputs/llm_bin_cb.csv', limit=limit)
...
def run_llm_quad_cb(limit=None):
    def prompt_fn(sentence):
        return (f"You are a political event classifier. Classify the relation "
                f"between source (<S></S>) and target (<T></T>).\n\n"
                f"LABEL DEFINITIONS:\n{CODEBOOK_QUADCODE}\n\n"
    ...
    return run_llm_quad_experiment("LLM Quad CB", prompt_fn,
                              f'{REPO}/outputs/llm_quad_cb.csv', limit=limit)
...
def main():
    ...
    parser.add_argument('--step', required=True,
        choices=['tree','tiny','full','llm_no_cb','llm_cb','llm_cot','llm_icl',
                 # === ADDED ===
                 'llm_bin_no_cb','llm_bin_cb','llm_bin_cot','llm_bin_icl',
                 'llm_quad_no_cb','llm_quad_cb','llm_quad_cot','llm_quad_icl',
    ...
    dispatch = {
        ...
        'llm_bin_cb':     lambda: run_llm_bin_cb(args.limit),
        ...
        'llm_quad_cb':    lambda: run_llm_quad_cb(args.limit),
